...
header "Yabridge GUI Controller Package Installation Tests"
...
if [[ "$choices" == *"1"* ]]; then
    ...
    for ver in $(conf_versions "fedora-versions"); do
        pkg="$(ls "$OUTPUT"/yabridge-gui-controller-*.fc${ver}.noarch.rpm 2>/dev/null | head -1)"
        if [[ -n "$pkg" ]]; then
...
if [[ "$choices" == *"2"* ]]; then
    ...
    for ver in $(conf_versions "opensuse-versions"); do
        pkg="$(ls "$OUTPUT"/yabridge-gui-controller-*.opensuse*.noarch.rpm 2>/dev/null | head -1)"
        if [[ -n "$pkg" ]]; then
...
if [[ "$choices" == *"3"* ]]; then
    ...
    for ver in $(conf_versions "debian-versions"); do
        pkg="$(ls "$OUTPUT"/yabridge-gui-controller_*~${ver}.deb 2>/dev/null | head -1)"
        if [[ -n "$pkg" ]]; then
...
if [[ "$choices" == *"5"* ]]; then
    header "Arch package"
    pkg="$(ls "$OUTPUT"/arch/yabridge-gui-controller-*.pkg.tar.zst 2>/dev/null | head -1)"
    if [[ -n "$pkg" ]]; then
...
if [[ "$choices" == *"6"* ]]; then
    ...
    pkg="$(ls "$OUTPUT"/yabridge-gui-controller-*-linux.tar.gz 2>/dev/null | head -1)"
    if [[ -z "$pkg" ]]; then
